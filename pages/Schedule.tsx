...
const Schedule: React.FC = () => {
  ...
  const groupedMatches = currentMatches.reduce(
    (groups, match) => {
          ...
          {sortedDates.length > 0 ? (
              ...
              >
                ...
                <div className="grid gap-4">
                  {groupedMatches[date].map((match) => {
                    const totalVotes =
                      (match.homeVotes || 0) + (match.awayVotes || 0);
                    const homePercent = calculatePercentage(
                      match.homeVotes,
                      totalVotes,
                    );
                    const awayPercent = calculatePercentage(
                      match.awayVotes,
                      totalVotes,
                    );
                          ...
                          <div className="w-full md:w-1/4 flex flex-row md:flex-col items-center md:items-end justify-between md:justify-center gap-3 mt-2 md:mt-0 border-t md:border-t-0 border-gray-100 dark:border-gray-700 pt-4 md:pt-0">
                            ...
                            <div className="flex items-center gap-2">
                              {match.status === "Live" ? (
                                <Button
                                  size="sm"
                                  className="w-full md:w-auto h-9 text-xs gap-1 shadow-md shadow-saudi-green/20"
                                >
            ...
            <div className="flex flex-col items-center justify-center py-20 text-center">
              ...
              <p className="text-gray-500 dark:text-gray-400 mt-2">
                Check back later for updated fixtures.
              </p>
            </div>
          )}
        </div>
